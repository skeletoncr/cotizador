...
.factory("impresionClienteService", function(crediq, calcularFormulasService) {
  var factory = {};
  var datos = {};
  ...
  function format(n, currency) {
	...
	factory.impresionClientePDF = function(detalle, extrasCotizacion, extrasPromocion) {
    ...
    datos.subTotalSeguro = calcularFormulasService.calcularSubTotalSeguro(detalle.cambio_dolar, detalle.cambio_dolar_seguros, extrasCotizacion, extrasPromocion, detalle.nombreMoneda, detalle.costovehiculo, detalle.a, detalle.b, detalle.c, detalle.d, detalle.f, detalle.g, detalle.h);
    //calcula el financiamiento de seguro
    datos.financiamientoSeguro = calcularFormulasService.calcularFinanciamientoSeguro(detalle.cambio_dolar_seguros, detalle.nombreMoneda, datos.subTotalSeguro, detalle.bonificacionSeguro);
    //calcula el seguro de vehiculo
    datos.seguroVehiculo = calcularFormulasService.calcularSeguroVehiculo(detalle.cambio_dolar_seguros, detalle.nombreMoneda, datos.subTotalSeguro, detalle.bonificacionSeguro);
    ...
    var bodyConExtras = [
                [{ text: 'Financiamiento en:', style: 'tableHeader' }, { text: detalle.nombreMoneda, style: 'tableHeader'}, { text: 'Porcentajes', style: 'tableHeader' }],
    ...
    var docDefinition = {
      content: [
        ...
        {
          text: 'En atención a su estimable solicitud de financiamiento, ' 
          + 'nos permitimos presentarle una oferta formal para la compra de su '
          + 'vehículo bajo las siguientes condiciones de crédito, las cuales esperamos cumplan '
        ...
        {
          text: 'Plazo del Crédito en Meses: ' + detalle.plazo1,
          fontSize: 7,
          bold: true,
          margin: [100, 5, 0, 15]
        },
        ...
        tablaBallon,
        {
          ...
          table: {
            ...
            body: [
              ...
              [ 'Requisitos', '' ],
              [ 'Solicitante Persona Física', 'Solicitante Jurídico' ],
              [ 
                '1 - Solicitud de Crédito debidamente llenada y firmada.\n'
                +'2 - Copia de cédula o documento de identidad vigente.\n'
                +'* Copia de un recibo de servicio público con la dirección.\n'
                +'Para Asalariados\n'
                +'a - Constancia salarial con menos de un mes de emitida.\n'
                +'b - Copia de la ultima Orden Patronal que este vigente.\n'
                +'Para no Asalariados\n'
                +'a - Constancia de CPA. con menos de un mes de emitida\n'
                +'(El CPA debe venir con su respectiva hoja de trabajo, certificación un año)\n'
                +'* Estados de Cuenta Bancarios mínimo de últimos 3 meses\n'
                + '* Facturas, recibos o documentos que justifiquen CPA de 3 meses si la prima es inferior al 40%', 
                '1 - Solicitud de Crédito debidamente llenada y firmada.\n'
                +'2 - Copia de cédula Jurídica.\n'
                +'3 - Certificación Original de Personería Jurídica vigente.\n'
                +'4 - Copia de cédula o documento de identidad del apoderado.\n'
                +'5 - Copia de un recibo de servicio público con la dirección.\n'
                +'6 - Estados Financieros Certificados de los dos últimos periodos, certificados por un CPA. Y un corte reciente de no más de tres meses de emitido.\n'
                +'* Declaración de Ventas o de Renta de últimos tres periodos.\n'
                +'* Estados de Cuenta Bancarios mínimo de últimos 3 meses.'
              ],
            ]
          },
        ...
        {
          text: detalle.user.nombre + ' ' + detalle.user.apellido1 + '          ' + detalle.user.email + '          ' + detalle.user.telefono,
          fontSize: 7,
          bold: true,
          margin: [100, 5, 0, 15]
        }
  ...
  function getMontoExtra(detalle, extra){
    ...
    if(detalle.id_moneda == 1 && extra.moneda == 2){
      valor = extra.monto / parseFloat(detalle.cambio_dolar);
    }
  ...
  function subsidioSeguroYSubsidioTasa(detalle){
    if(parseFloat(detalle.subsidioSeguro) == 12){
      ...
      cuotaFinal1 = (parseFloat(strPMT1) + parseFloat(seguroDeudaPrimerAnnio) + 0).toFixed(2);
      //valor total de la cuota 2
      cuotaFinal2 = (parseFloat(strPMT2) + parseFloat(seguroDeudaOtroAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
      //valor total de la cuota 3
      cuotaFinal3 = (parseFloat(strPMT3) + parseFloat(seguroDeudaUltimoAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
  ...
  function soloPeriodoGracia(detalle){
    ...
    cuotaFinal1 = (parseFloat(seguroDeudaPrimerAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
    cuotaFinal2 = (parseFloat(strPMT2) + parseFloat(seguroDeudaOtroAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
    cuotaFinal3 = (parseFloat(strPMT3) + parseFloat(seguroDeudaUltimoAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
    datos.salarioMinimo = calcularFormulasService.calculaSalarioMinimo(parseFloat(cuotaFinal3), parseFloat(detalle.rcimaximo));
    tabla = 
  ...
  function tablaTasasIguales(detalle){
    ...
    //valor total de la cuota 1
    cuotaFinal1 = (parseFloat(strPMT1) + parseFloat(seguroDeudaPrimerAnnio) + parseFloat(datos.seguroVehiculo)).toFixed(2);
    //calcular salario minimo para poder pagar la cuota
