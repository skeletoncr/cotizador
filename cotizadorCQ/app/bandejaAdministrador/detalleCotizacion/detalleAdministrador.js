angular
  .module("Cotizador")
  .controller('DetalleAdministradorController', DetalleAdministradorController);

/* @ngInject */
function DetalleAdministradorController($scope, $rootScope, $location, $http, $routeParams, $q, 
  ...
  $scope.openClientePdf = function() {
    ...
    if($scope.tipoPrestamo == '0'){
      getDetallePDF().then(function(data) {
        if($scope.comision != -1){
          $scope.detallesPDF.comisionBancaria = $scope.comision.toString();
        }
        $scope.detallesPDF.seguroTotal = $scope.seguroTotal;
        $scope.detallesPDF.seguroMensual = $scope.seguroMensual;
  ...
  function getPlazosDeBandeja(){
    $scope.plazosPromocion = [];
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 84).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 72).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 60).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 48).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 36).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 24).toString());
    $scope.plazosPromocion.push((parseFloat($scope.selectedPromocion.plazo1) + 12).toString());
    $scope.plazosPromocion.push($scope.selectedPromocion.plazo1);
    ...
    $scope.plazosPromocion.push(($scope.selectedPromocion.plazo1 - 48).toString());
    $scope.plazosPromocion.push(($scope.selectedPromocion.plazo1 - 60).toString());
    $scope.plazosPromocion.push(($scope.selectedPromocion.plazo1 - 72).toString());
    $scope.plazosPromocion.push(($scope.selectedPromocion.plazo1 - 84).toString());
    ...
    $http.post("app/bandejaEntrada/detalleCotizacion/php/getDetalleCotizacion.php", {idCaso: $routeParams.idCaso})
      .success(function(data){
        if(angular.isObject(data)){
          ...
          if($scope.nombreMarca == 'CREDIAUTOS' || $scope.nombreMarca == 'NOVACIONES'){
            $scope.esUsado = true;
          }else{
            $scope.esUsado = false;
          }
