...
/* @ngInject */
function VendedorController($scope, $rootScope, loading, $location, $http, $q, correoService, calcularFormulasService, impresionClienteService, impresionPreAprobacionService, impresionNoPreAprobacionService, impresionClienteLeasingFinancieroService, impresionClienteLeasingOperativoService) {
      ...
      if($scope.tipoPrestamo == '0'){
        getDetallePDF().then(function(data) {
          getExtrasPDF().then(function(data) {
            ...
            detalleImpresion.bonificacionSeguro = '-45';
  ...
  function calcularCuotaPrendario(){
    ...
    var financiamientoSeguro = calcularFormulasService.calcularFinanciamientoSeguro($scope.detallesPDF.cambio_dolar_seguros, $scope.detallesPDF.nombreMoneda, subTotalSeguro, "-45");
    var seguroVehiculo = calcularFormulasService.calcularSeguroVehiculo($scope.detallesPDF.cambio_dolar_seguros, $scope.detallesPDF.nombreMoneda, subTotalSeguro, "-45");
  ...
  function getDetalleLeasingPDF(){
    ...
    $http.post("app/vendedor/php/getDetalleLeasingPDF.php", variables)
      .success(function(data){
        if(angular.isObject(data)){
          var detallePDF = JSON.parse(data[0]);
          $scope.detallesPDF = detallePDF[0];
  ...
  function getMarcasDeUsuario(){
    ...
    .success(function(data){
      if(angular.isObject(data)){
        ...
        $scope.marcas = marcas;
        $scope.selectedMarca = $scope.marcas[0];
        $scope.onMarcasChange();
        $scope.marcasLoading = false;
        defered.resolve();
      }else {
        $scope.marcas = [];
  ...
  function subsidioSeguroYSubsidioTasa(detalle){
    if(parseFloat(detalle.subsidioSeguro) == 12){
      ...
      strPMT3 = calcularFormulasService.PMT(TasaResult3, detalle.plazo3, calcularFormulasService.calculaPagoPeriodo($scope.saldoAFinanciar, detalle.tasa2, plazo2Diferencia, strPMT2)) * -1;
...
VendedorController.$inject = ['$scope', '$rootScope', 'loading', '$location', '$http', '$q', 'correoService', 'calcularFormulasService', 'impresionClienteService', 'impresionPreAprobacionService', 'impresionNoPreAprobacionService', 'impresionClienteLeasingFinancieroService', 'impresionClienteLeasingOperativoService'];
